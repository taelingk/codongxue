...
class TrendDialog(QDialog):
    ...
    def __init__(self, history_data, parent=None):
        ...
        # 每个点大约给 0.02 inch（dpi=100 -> 2px），最多 60 inch（6000px），防止无限变大卡顿
        ...
        # ---- 用 ScrollArea 包住画布：时间很长也能横向滚动回看前面 ----
        container = QWidget()
        ...
        scroll.setWidgetResizable(False)  # 不让它自动压缩画布，否则看不出“前面数据”
    ...
    def plot_trend(self):
        ...
        # 数据很长时：只显示部分 marker，避免太卡（线仍然是全量）
        mark_step = max(1, len(times) // 200)  # 大概最多显示 200 个点的 marker
...
class PulseWaveMonitor(QMainWindow):
    ...
    def calculate_five_second_average(self):
        ...
        # ✅ 不再截断 history，保证趋势/保存从 0 秒开始完整保留
