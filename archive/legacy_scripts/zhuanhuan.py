...
# 配置Windows系统中文字体（使用系统自带字体）
plt.rcParams["font.family"] = ["SimHei", "Microsoft YaHei", "Microsoft JhengHei", "Arial Unicode MS"]
plt.rcParams["axes.unicode_minus"] = False  # 解决负号显示问题
plt.rcParams['font.sans-serif'] = ["SimHei", "Microsoft YaHei"]  # 再次强化字体设置
...
# ========== 新增：统一数值类型转换工具函数 ==========
def to_python_num(n):
    """将numpy数值类型转为Python原生类型（避免timedelta类型错误）"""
    if isinstance(n, (np.integer, np.floating)):
        return float(n) if isinstance(n, np.floating) else int(n)
    return n
...
def standardize_single_feature(data, scalar_path):
    ...
    try:
        ...
        data_reshaped = data.reshape(-1, 1)  # 转为2维输入
...
class PulseWaveMonitor(QMainWindow):
    ...
    def __init__(self):
        ...
        # 新增：1秒级推理缓存和5秒均值相关
        self.one_second_predictions = []  # 存储1秒级的CO/SV推理结果
        self.five_second_update_interval = 5  # 5秒更新一次界面
        self.last_five_second_update = 0  # 上次5秒更新的时间点
    ...
    def init_model_paths(self):
        ...
        # 模型根目录
        model_root_dir = r"D:/心输出量项目/CNAP脉氧处理数据/脉氧+CNAP/resnet_LSTM_SE/20251221_210322"

        self.model_config = {
            'model_path': os.path.join(model_root_dir, 'resnet18_lstm_se_model.onnx'),  # ONNX模型文件
    ...
    def load_inference_model(self):
        ...
        try:
            ...
            for input_info in self.onnx_session.get_inputs():
                logging.info(f"输入{input_info.name} 形状: {input_info.shape} 类型: {input_info.type}")
    ...
    def init_ui(self):
        ...
        self.title_label = QLabel("心功能参数趋势图（每5秒更新一次）")
        ...
        result_layout.addWidget(self.update_status_label)
    ...
    def init_data(self):
        ...
        self.scroll_timer.setInterval(self.scroll_step * 1000)  # 1秒滚动一次
        self.scroll_timer.timeout.connect(self.scroll_window)

        # 修改：预测定时器改为1秒一次（用于1秒级推理）
        self.predict_timer = QTimer(self)
        self.predict_timer.setInterval(1000)  # 1秒推理一次
    ...
    def process_loaded_data(self):
        ...
        self.peaks_history = []
        self.prediction_history = []
        self.one_second_predictions = []  # 重置1秒级缓存
        self.last_five_second_update = 0
    ...
    def detect_valley_points(self, data, sampling_rate=125):
        ...
        # 检测谷值（使用find_peaks检测负值的峰值）
        valleys, _ = find_peaks(-data_filtered,
                                distance=int(sampling_rate * 60 / 200),  # 最小谷值间隔
                                prominence=0.5)  # 谷值突出度

        return valleys

    def get_valley_aligned_data(self, start_time):
        """获取以谷值为起始点的1秒数据"""
        # 计算当前时间点前后2秒的范围，用于查找最近的谷值
        search_start = max(0, int((start_time - 2) * self.sampling_rate))
        search_end = min(len(self.pulse_data), int((start_time + 2) * self.sampling_rate))

        if search_end - search_start < self.sampling_rate:
            return None

        # 检测该范围内的谷值点
        search_data = self.pulse_data[search_start:search_end]
        valleys = self.detect_valley_points(search_data, self.sampling_rate)

        if len(valleys) == 0:
            # 未检测到谷值，返回默认1秒数据
            start_idx = int(start_time * self.sampling_rate)
            end_idx = start_idx + self.sampling_rate
            if end_idx > len(self.pulse_data):
                return None
            return self.pulse_data[start_idx:end_idx]

        # 找到最接近目标时间的谷值点
        target_idx = int(start_time * self.sampling_rate) - search_start
        closest_valley = min(valleys, key=lambda x: abs(x - target_idx))
        valley_global_idx = search_start + closest_valley

        # 从谷值点开始截取1秒数据
        end_idx = valley_global_idx + self.sampling_rate
        if end_idx > len(self.pulse_data):
            return None

        return self.pulse_data[valley_global_idx:end_idx]
    ...
    def plot_current_window(self):
        ...
        # 设置时间标签 - 修复：将numpy类型转为Python原生类型
        xticks = np.arange(
        ...
        xtick_labels = [
            (self.base_time + timedelta(seconds=to_python_num(t))).strftime("%H:%M:%S")
            for t in xticks
        ]
    ...
    def scroll_window(self):
        ...
        # 更新状态 - 修复：将numpy类型转为Python原生类型
        start_actual = self.base_time + timedelta(seconds=to_python_num(self.current_window_start))
        end_actual = self.base_time + timedelta(seconds=to_python_num(next_end))
    ...
    def start_scrolling(self):
        ...
        self.predict_timer.start()  # 启动1秒级推理定时器
        self.normalize_label.setText("参数标准化状态: 已完成")

        self.model_status.setText("模型状态：ONNX模型运行中（每1秒推理一次，每5秒更新显示）")
        self.update_status_label.setText("更新状态: 1秒级推理中...")

        start_actual = self.base_time + timedelta(seconds=to_python_num(self.current_window_start))
        end_actual = self.base_time + timedelta(seconds=to_python_num(self.window_size))
        self.status_label.setText(
            f"状态：预测中 当前窗口：{start_actual.strftime('%H:%M:%S')} - {end_actual.strftime('%H:%M:%S')}"
        )
    ...
    def pause_scrolling(self):
        ...
        start_actual = self.base_time + timedelta(seconds=to_python_num(self.current_window_start))
        end_actual = self.base_time + timedelta(seconds=to_python_num(self.current_window_start + self.window_size))
        self.status_label.setText(
    ...
    def save_data(self):
        ...
        if self.one_second_predictions:
            one_second_data = pd.DataFrame({
                ...
                "心率(次/分)": [self.calculated_hr] * len(self.one_second_predictions)
            })

        # 合并数据
        save_data = pd.concat([five_second_data, one_second_data], ignore_index=True)

        # 添加生理参数
        save_data["体重(kg)"] = self.weight_input.text()
        save_data["年龄(岁)"] = self.age_input.text()
        save_data["身高(cm)"] = self.height_input.text()
        save_data["性别"] = "男" if self.male_btn.isChecked() else "女" if self.female_btn.isChecked() else "其他"
        ...
        if file_path:
            try:
                with pd.ExcelWriter(file_path, engine='openpyxl') as writer:
                    save_data.to_excel(writer, sheet_name='所有数据', index=False)
    ...
    def prepare_model_inputs(self, pulse_data_1s=None):
        """准备ONNX模型输入数据（支持指定1秒数据）"""
        try:
            # 获取1秒数据（优先使用传入的谷值对齐数据）
            if pulse_data_1s is None:
                ...
                window_pulse = pulse_data_1s[:125]
                if len(window_pulse) < 125:
                    window_pulse = np.pad(window_pulse, (0, 125 - len(window_pulse)), 'constant')

            # 重塑为模型输入格式
            raw_signal = window_pulse.reshape(1, -1)  # [1, 125]
            ...
            # 标准化特征
            X_reshaped = X.reshape(X.shape[0], -1)  # [1, 375]
            X_std = self.signal_scaler.transform(X_reshaped).reshape(X.shape)  # [1, 125, 3]

            # 标准化标量特征并转换为2维张量 [1, 1]
            age_std = standardize_single_feature(np.array([age]), self.scalar_paths['age'])
            age_std = age_std.reshape(1, 1).astype(np.float32)  # 关键修复：转为2维

            weight_std = standardize_single_feature(np.array([weight]), self.scalar_paths['weight'])
            weight_std = weight_std.reshape(1, 1).astype(np.float32)

            height_std = standardize_single_feature(np.array([height]), self.scalar_paths['height'])
            height_std = height_std.reshape(1, 1).astype(np.float32)

            bsa_std = standardize_single_feature(np.array([bsa]), self.scalar_paths['bsa'])
            bsa_std = bsa_std.reshape(1, 1).astype(np.float32)

            bmi_std = standardize_single_feature(np.array([bmi]), self.scalar_paths['bmi'])
            bmi_std = bmi_std.reshape(1, 1).astype(np.float32)

            hr_std = standardize_single_feature(np.array([hr]), self.scalar_paths['hr'])
            hr_std = hr_std.reshape(1, 1).astype(np.float32)

            # 性别特征转为2维张量 [1, 1]
            gender_arr = np.array([gender], dtype=np.float32).reshape(1, 1)

            # 转换为float32（ONNX常用格式）
            X_std = X_std.astype(np.float32)

            # 打印输入形状调试信息
            logging.debug(f"signal_input形状: {X_std.shape}")
            logging.debug(f"age_input形状: {age_std.shape}")
            logging.debug(f"gender_input形状: {gender_arr.shape}")
    ...
    def predict_co_sv(self, pulse_data_1s=None):
        ...
        try:
            ...
            input_feed = {
                'signal_input': X_std,
                'age_input': age_std,
                'gender_input': gender_arr,
                'weight_input': weight_std,
                'height_input': height_std,
                'bsa_input': bsa_std,
                'bmi_input': bmi_std,
                'hr_input': hr_std
            }
            ...
            for name, data in input_feed.items():
                logging.debug(f"输入{name} 形状: {data.shape} 类型: {data.dtype}")
    ...
    def update_one_second_prediction(self):
        ...
        try:
            # 获取以谷值为起始点的1秒数据
            current_time_point = self.current_window_start + (self.window_size / 2)  # 窗口中间点
            pulse_data_1s = self.get_valley_aligned_data(current_time_point)

            if pulse_data_1s is None:
                self.update_status_label.setText("更新状态: 无法获取谷值对齐数据，使用默认切片")
                # 使用默认1秒数据
            ...
            # 如果预测值为0（模型未加载或预测失败），不更新
            if co == 0 and sv == 0:
                self.model_status.setText("模型状态：1秒推理失败（ONNX模型未加载或输入数据错误）")
                self.model_status.setStyleSheet("color: red;")
                return

            # 记录1秒级推理结果 - 修复：将numpy类型转为Python原生类型
            current_time = self.base_time + timedelta(seconds=to_python_num(self.current_window_start))
            self.one_second_predictions.append((current_time, co, sv))

            # 限制1秒级缓存长度（最多保存100个）
            if len(self.one_second_predictions) > 100:
                self.one_second_predictions = self.one_second_predictions[-100:]

            # 检查是否需要更新5秒均值 - 修复：将numpy类型转为Python原生类型
            current_second = int(to_python_num(self.current_window_start))
            if current_second - self.last_five_second_update >= self.five_second_update_interval:
                self.calculate_five_second_average()
                self.last_five_second_update = current_second

        except Exception as e:
            self.model_status.setText(f"模型状态：1秒推理出错 - {str(e)}")
            self.model_status.setStyleSheet("color: red;")
            logging.error(f"1秒推理更新失败: {e}")
            import traceback
            logging.error(f"详细错误堆栈: {traceback.format_exc()}")
    ...
    def calculate_five_second_average(self):
        ...
        # 计算均值
        co_avg = round(np.mean(co_values), 2)
        sv_avg = round(np.mean(sv_values), 1)
        ...
        self.co_value.setStyleSheet(
            ...
            if not (4 <= co_avg <= 8) else "font-size: 24pt; font-weight: bold;"
        )
        self.sv_value.setStyleSheet(
            "font-size: 24pt; font-weight: bold; color: red;"
            if not (60 <= sv_avg <= 100) else "font-size: 24pt; font-weight: bold;"
        )

        # 保存5秒均值历史 - 修复：将numpy类型转为Python原生类型
        current_time = self.base_time + timedelta(seconds=to_python_num(self.current_window_start))
        ...
        self.model_status.setStyleSheet("color: green;")
        self.update_status_label.setText(f"更新状态: 5秒均值已更新（基于最近5个