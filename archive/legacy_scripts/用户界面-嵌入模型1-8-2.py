...
from collections import deque  # 关键导入
from scipy.signal import find_peaks, butter, filtfilt, resample, detrend  # 新增detrend做基线校正
...
def resample_waveform(data, original_fs=225, target_fs=125):
    ...
    return resampled_data.astype(np.float32), target_time


def generate_time_axis(data_length, fs, start_time=None):
    """
    生成波形的时间轴（横轴为实际时间，仅显示HH:MM:SS）
    修正：基于实际采样率计算时间，确保时间轴准确
    :param data_length: 数据点数量
    :param fs: 实际采样率（Hz）
    :param start_time: 起始时间（datetime对象）
    :return: 时间轴数组（datetime对象）、时间标签列表、数值时间轴（秒）
    """
    # 计算数值时间轴（秒）
    time_seconds = np.arange(data_length) / fs
    # 确定起始时间（默认当前时间 - 总时长）
    if start_time is None:
        start_time = datetime.now() - timedelta(seconds=time_seconds[-1] if len(time_seconds) > 0 else 0)
    # 生成时间轴（每个采样点对应一个时间）
    time_axis = []
    time_labels = []
    ...
    return time_axis, time_labels, time_seconds
...
class SerialReceiveThread(QThread):
    ...
    def parse_spo2_bytes_to_wave(self):
        ...
        if temp_red_wave:
            ...
            # 1. 225Hz数据预处理（取反+滤波+归一化+基线校正）
            wave_inverted = -raw_wave
            wave_detrend = detrend(wave_inverted)  # 新增：基线校正
            wave_filtered = self.butter_bandpass_filter(wave_detrend)
            wave_225hz = self.normalize_wave(wave_filtered)

            # 2. 降采样到125Hz（供模型使用）
            wave_125hz, _ = resample_waveform(wave_225hz, self.FS_225, self.FS_125)
...
class TrendDialog(QDialog):
    def __init__(self, history_data, parent=None):
        ...
        self.fig = Figure(figsize=(18,10), dpi=300)
...
class PulseWaveMonitor(QMainWindow):
    ...
    def init_config(self):
        ...
        self.pulse_time_125hz = None  # 125Hz数据对应的时间轴（秒）
    ...
    def init_ui(self):
        ...
        self.setGeometry(100, 100, 1600, 1000)  # 调大窗口尺寸
        ...
        self.fig = Figure(figsize=(8, 4), dpi=100)  # 调大图表尺寸
    ...
    def load_from_file(self):
        ...
        try:
            ...
            # 降采样到125Hz（保留时间轴）
            self.pulse_data_125hz, self.pulse_time_125hz = resample_waveform(self.pulse_data_225hz, 225, 125)
    ...
    def on_serial_data_125hz(self, wave_data):
        ...
        # 生成对应的时间轴
        self.pulse_time_125hz = np.arange(len(self.pulse_data_125hz)) / self.sampling_rate_125
    ...
    def calculate_heart_rate(self):
        ...
        修正：精准计算心率
        1. 基线校正 + 滤波，提高峰值检测准确性
        2. 动态调整峰值检测参数，适配正常心率范围（60-120次/分）
        3. 异常值过滤，只保留合理的峰间距
        """
        ...
        # 1. 数据预处理：基线校正 + 滤波
        wave_data = self.pulse_data_225hz.copy()
        wave_detrend = detrend(wave_data)  # 基线校正
        # 2. 动态计算峰值检测参数（适配60-120次/分的心率）
        min_distance = int(self.sampling_rate_225 / 120 * 60)  # 最小峰间距（120次/分对应的采样点数）
        max_distance = int(self.sampling_rate_225 / 60 * 60)  # 最大峰间距（60次/分对应的采样点数）
        # 3. 峰值检测（放宽prominence，提高检测率）
        peaks, properties = find_peaks(
            wave_detrend,
            distance=min_distance,
            prominence=np.std(wave_detrend) * 0.05,  # 动态阈值
            width=(1, max_distance // 2)
        )
        logging.info(f"检测到峰值数量：{len(peaks)} | 225Hz数据长度：{len(wave_data)}")

        if len(peaks) >= 2:
            # 4. 计算峰间距并过滤异常值
            intervals = np.diff(peaks) / self.sampling_rate_225  # 峰间时间（秒）
            # 过滤异常值：只保留0.5-1.0秒的峰间距（对应60-120次/分）
            valid_intervals = intervals[(intervals >= 0.5) & (intervals <= 1.0)]
            logging.info(f"原始峰间时间：{intervals} | 有效峰间时间：{valid_intervals}")

            if len(valid_intervals) >= 2:
                avg_interval = np.mean(valid_intervals)
                self.calculated_hr = int(60 / avg_interval)
            else:
                # 若无有效区间，用原始均值（兼容边缘情况）
        ...
        # 5. 限制心率范围（医学合理值）
        self.calculated_hr = np.clip(self.calculated_hr, 40, 180)
        self.hr_value.setText(f"{self.calculated_hr}")

    def update_wave_display(self):
        """
        修正：显示5秒窗口的脉搏波波形
        1. 基于125Hz数据和对应的时间轴绘制，保证时间准确
        2. 时间轴标签与实际采样率匹配
        """
        if self.pulse_data_125hz is None or len(self.pulse_data_125hz) == 0:
            self.ax.clear()
            self.ax.text(0.5, 0.5, "暂无数据", ha='center', va='center', transform=self.ax.transAxes, fontsize=14)
            self.canvas.draw()
            return

        # ========== 1. 截取最近5秒的125Hz数据（625个点） ==========
        target_points = 5 * self.sampling_rate_125  # 5秒×125Hz=625个点
        if len(self.pulse_data_125hz) >= target_points:
            wave_display = self.pulse_data_125hz[-target_points:]
            # 对应的时间轴（秒）
            if self.pulse_time_125hz is not None and len(self.pulse_time_125hz) >= target_points:
                time_display = self.pulse_time_125hz[-target_points:]
            else:
                time_display = np.arange(target_points) / self.sampling_rate_125
            # 起始时间：当前时间 - 5秒
            start_time = datetime.now() - timedelta(seconds=5)
        else:
            # 数据不足5秒时，补0到625个点
            wave_display = np.pad(self.pulse_data_125hz, (0, target_points - len(self.pulse_data_125hz)), 'constant')
            time_display = np.arange(len(wave_display)) / self.sampling_rate_125
            # 起始时间：当前时间 - 实际数据时长
            data_duration = len(self.pulse_data_125hz) / self.sampling_rate_125
            start_time = datetime.now() - timedelta(seconds=data_duration)

        # ========== 2. 生成时间轴标签（HH:MM:SS） ==========
        time_axis, time_labels, _ = generate_time_axis(
        ...
        self.ax.clear()
        # 绘制125Hz降采样后的波形（加粗线条）
        self.ax.plot(time_display, wave_display, 'r-', linewidth=2, label='脉搏波（125Hz）')
        ...
        tick_positions = [time_display[i] for i in tick_indices]
