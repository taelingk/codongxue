...
def split_data_by_time_groups(X, y, age, gender, weight, height, bsa, bmi, hr, sbp, dbp, pp, sv, time, indices,
    ...
    def extract(mask):
        return (
            ...
            sbp[mask], dbp[mask], pp[mask],
            time[mask], indices[mask]
        )

    X_train, y_train, age_train, gender_train, weight_train, height_train, bsa_train, bmi_train, hr_train, sbp_train, dbp_train, pp_train, time_train, indices_train = extract(
        train_mask)
    X_val, y_val, age_val, gender_val, weight_val, height_val, bsa_val, bmi_val, hr_val, sbp_val, dbp_val, pp_val, time_val, indices_val = extract(
        val_mask)
    X_test, y_test, age_test, gender_test, weight_test, height_test, bsa_test, bmi_test, hr_test, sbp_test, dbp_test, pp_test, time_test, indices_test = extract(
        test_mask)
...
def plot_bland_altman(y_true, y_pred, metric_name, save_path):
    ...
    plt.axhline(y=lower_limit, color='#F18F01', linestyle='--', linewidth=1.5,
                label=f'95% LoA: [{lower_limit:.2f}, {upper_limit:.2f}]')
...
# ================== 新增：标准化层（把 z=(x-μ)/σ 写进图里） ==================
class Standardize1D(tf.keras.layers.Layer):
    """
    对 shape=(None, 1) 或 (None,) 的数值做 (x-mean)/scale
    mean/scale 为常量（来自 sklearn StandardScaler）
    """
    def __init__(self, mean, scale, eps=1e-8, **kwargs):
        super().__init__(**kwargs)
    ...
    def get_config(self):
        ...
        cfg.update({"mean": self.mean.reshape(-1).tolist(), "scale": self.scale.reshape(-1).tolist(), "eps": self.eps})
        return cfg


class StandardizeSignalFlat(tf.keras.layers.Layer):
    """
    对 signal_input shape=(None,125,3) 做与 sklearn StandardScaler(is_2d=True) 完全一致的标准化：
    先 reshape -> (None, 375)，逐维 (x-mean)/scale，再 reshape 回 (None,125,3)
    """
    def __init__(self, mean_375, scale_375, eps=1e-8, **kwargs):
        super().__init__(**kwargs)
        self.mean = np.array(mean_375, dtype=np.float32).reshape(1, -1)   # (1,375)
        self.scale = np.array(scale_375, dtype=np.float32).reshape(1, -1) # (1,375)
        self.eps = float(eps)

    def call(self, x):
        x = tf.cast(x, tf.float32)
        b = tf.shape(x)[0]
        x2 = tf.reshape(x, (b, -1))  # (None,375)
        x2 = (x2 - self.mean) / (self.scale + self.eps)
        return tf.reshape(x2, (b, 125, 3))

    def get_config(self):
        cfg = super().get_config()
        cfg.update({"eps": self.eps})
        # mean/scale 太长，不建议写入 config 里打印；导出时仍会作为常量保存到图里
        return cfg


# ================== 重要：构建“带内置标准化”的模型（不改你的主干结构） ==================
def build_resnet18_with_se_lstm_with_standardization(
    input_shape,
    x_scaler, age_scaler, weight_scaler, height_scaler, bsa_scaler, bmi_scaler, hr_scaler, sbp_scaler, dbp_scaler, pp_scaler
):
    """
    只是在模型最前面插入标准化层：
    - signal_input: 用 375 维 mean/scale 标准化后再送入原网络
    - 各临床特征：用各自 mean/scale 标准化后再 concat
    其余网络结构与你的 build_resnet18_with_se_lstm 完全一致（照抄）
    """
    best_hp = {
        "lr": 0.001,
    ...
    # 原始输入（不再要求你外面先标准化）
    inputs_raw = Input(shape=input_shape, name="signal_input")
    age_raw = Input(shape=(1,), name="age_input")
    gender_in = Input(shape=(1,), name="gender_input")  # 你原来没标准化 gender，这里保持不变
    weight_raw = Input(shape=(1,), name="weight_input")
    height_raw = Input(shape=(1,), name="height_input")
    bsa_raw = Input(shape=(1,), name="bsa_input")
    bmi_raw = Input(shape=(1,), name="bmi_input")
    hr_raw = Input(shape=(1,), name="hr_input")
    sbp_raw = Input(shape=(1,), name="sbp_input")
    dbp_raw = Input(shape=(1,), name="dbp_input")
    pp_raw = Input(shape=(1,), name="pp_input")

    # === 内置标准化（关键） ===
    x = StandardizeSignalFlat(x_scaler.mean_, x_scaler.scale_, name="signal_standardize")(inputs_raw)
    age_in = Standardize1D(age_scaler.mean_, age_scaler.scale_, name="age_standardize")(age_raw)
    weight_in = Standardize1D(weight_scaler.mean_, weight_scaler.scale_, name="weight_standardize")(weight_raw)
    height_in = Standardize1D(height_scaler.mean_, height_scaler.scale_, name="height_standardize")(height_raw)
    bsa_in = Standardize1D(bsa_scaler.mean_, bsa_scaler.scale_, name="bsa_standardize")(bsa_raw)
    bmi_in = Standardize1D(bmi_scaler.mean_, bmi_scaler.scale_, name="bmi_standardize")(bmi_raw)
    hr_in = Standardize1D(hr_scaler.mean_, hr_scaler.scale_, name="hr_standardize")(hr_raw)
    sbp_in = Standardize1D(sbp_scaler.mean_, sbp_scaler.scale_, name="sbp_standardize")(sbp_raw)
    dbp_in = Standardize1D(dbp_scaler.mean_, dbp_scaler.scale_, name="dbp_standardize")(dbp_raw)
    pp_in = Standardize1D(pp_scaler.mean_, pp_scaler.scale_, name="pp_standardize")(pp_raw)

    # === 下面开始：完全照抄你原来的网络（输入换成 x） ===
    x = Conv1D(64, 7, strides=2, padding='same', kernel_regularizer=regularizer)(x)
    ...
    model = Model(
        inputs=[inputs_raw, age_raw, gender_in, weight_raw, height_raw, bsa_raw, bmi_raw, hr_raw,
                sbp_raw, dbp_raw, pp_raw],
        outputs=outputs
    )
...
def evaluate_final_model(model, X, y_true, hr, time_groups, dataset_name, save_dir):
    ...
    if dataset_name == "Test set":
        ...
        logging.info(
            f"SV - Mean Bias: {ba_sv_results['mean_bias']:.4f}, 95% LoA: [{ba_sv_results['lower_limit']:.4f}, {ba_sv_results['upper_limit']:.4f}]")
        logging.info(
            f"CO - Mean Bias: {ba_co_results['mean_bias']:.4f}, 95% LoA: [{ba_co_results['lower_limit']:.4f}, {ba_co_results['upper_limit']:.4f}]")
...
def export_model_to_onnx(model, save_path):
    """
    将Keras模型导出为ONNX格式 - 输入仍是原始特征（未标准化），标准化已在模型内部
    """
    try:
        input_spec = (
            ...
            tf.TensorSpec((None, 1), tf.float32, name="sbp_input"),
            tf.TensorSpec((None, 1), tf.float32, name="dbp_input"),
            tf.TensorSpec((None, 1), tf.float32, name="pp_input"),
        )

        onnx_model, _ = tf2onnx.convert.from_keras(model, input_signature=input_spec, opset=13)
        onnx.save(onnx_model, save_path)

        _ = InferenceSession(save_path)
...
def main():
    ...
    save_dir = f"D:/心输出量项目/CNAP脉氧处理数据/脉氧+CNAP/resnet_SE_LSTM/{timestamp}"
    os.makedirs(save_dir, exist_ok=True)
    logging.info(f"All results will be saved to: {save_dir}")

    # 1. 读取数据
    file_path = "D:/心输出量项目/CNAP脉氧处理数据/脉氧+CNAP/7.11-11.21脉氧+CNAP.csv"
    ...
    # 4. 标准化特征（仍然照旧：为了得到 scaler 参数，并保存 pkl；但训练时不再用 *_std 喂模型）
    logging.info("Standardizing features (to obtain scaler params)...")
    ...
    sbp_train_std, sbp_val_std, sbp_test_std, sbp_scaler = standardize_features(sbp_train, sbp_val, sbp_test)
    dbp_train_std, dbp_val_std, dbp_test_std, dbp_scaler = standardize_features(dbp_train, dbp_val, dbp_test)
    pp_train_std, pp_val_std, pp_test_std, pp_scaler = standardize_features(pp_train, pp_val, pp_test)

    # 保存scaler（可选：你推理端不需要了，但训练记录保留无妨）
    ...
    joblib.dump(sbp_scaler, f"{save_dir}/sbp_scaler.pkl")
    joblib.dump(dbp_scaler, f"{save_dir}/dbp_scaler.pkl")
    joblib.dump(pp_scaler, f"{save_dir}/pp_scaler.pkl")
    logging.info("All feature scalers saved successfully")

    # 5. 构建“带内置标准化”的模型（网络主体不变）
    logging.info("Building model (standardization inside the model graph)...")
    model, best_hp = build_resnet18_with_se_lstm_with_standardization(
        input_shape=(125, 3),
        x_scaler=x_scaler,
        age_scaler=age_scaler,
        weight_scaler=weight_scaler,
        height_scaler=height_scaler,
        bsa_scaler=bsa_scaler,
        bmi_scaler=bmi_scaler,
        hr_scaler=hr_scaler,
        sbp_scaler=sbp_scaler,
        dbp_scaler=dbp_scaler,
        pp_scaler=pp_scaler
    )
    ...
    # 6. 训练模型（关键变化：喂“原始特征”，不喂 *_std，因为标准化已在模型里做）
    ...
    history = model.fit(
        [X_train_augmented, age_train.reshape(-1, 1), gender_train.reshape(-1, 1), weight_train.reshape(-1, 1),
         height_train.reshape(-1, 1), bsa_train.reshape(-1, 1), bmi_train.reshape(-1, 1), hr_train.reshape(-1, 1),
         sbp_train.reshape(-1, 1), dbp_train.reshape(-1, 1), pp_train.reshape(-1, 1)],
        y_train[:, 0],
        epochs=50,
        batch_size=batch_size,
        validation_data=(
            [X_val, age_val.reshape(-1, 1), gender_val.reshape(-1, 1), weight_val.reshape(-1, 1),
             height_val.reshape(-1, 1), bsa_val.reshape(-1, 1), bmi_val.reshape(-1, 1), hr_val.reshape(-1, 1),
             sbp_val.reshape(-1, 1), dbp_val.reshape(-1, 1), pp_val.reshape(-1, 1)],
            y_val[:, 0]
        ),
        callbacks=callbacks,
        verbose=1
    ...
    pred_sv_test, pred_co_test = generate_predictions(
        model,
        X_test, age_test.reshape(-1, 1), gender_test.reshape(-1, 1), weight_test.reshape(-1, 1),
        height_test.reshape(-1, 1), bsa_test.reshape(-1, 1), bmi_test.reshape(-1, 1), hr_test.reshape(-1, 1),
        sbp_test.reshape(-1, 1), dbp_test.reshape(-1, 1), pp_test.reshape(-1, 1),
        hr_test
    )

    # 8. 保存包含预测结果的数据集（仍保存原始X_*，不改）
    ...
    test_data = [X_test, age_test.reshape(-1, 1), gender_test.reshape(-1, 1), weight_test.reshape(-1, 1),
                 height_test.reshape(-1, 1), bsa_test.reshape(-1, 1), bmi_test.reshape(-1, 1), hr_test.reshape(-1, 1),
                 sbp_test.reshape(-1, 1), dbp_test.reshape(-1, 1), pp_test.reshape(-1, 1)]
    val_data = [X_val, age_val.reshape(-1, 1), gender_val.reshape(-1, 1), weight_val.reshape(-1, 1),
                height_val.reshape(-1, 1), bsa_val.reshape(-1, 1), bmi_val.reshape(-1, 1), hr_val.reshape(-1, 1),
                sbp_val.reshape(-1, 1), dbp_val.reshape(-1, 1), pp_val.reshape(-1, 1)]
    train_data = [X_train_augmented, age_train.reshape(-1, 1), gender_train.reshape(-1, 1), weight_train.reshape(-1, 1),
                  height_train.reshape(-1, 1), bsa_train.reshape(-1, 1), bmi_train.reshape(-1, 1), hr_train.reshape(-1, 1),
                  sbp_train.reshape(-1, 1), dbp_train.reshape(-1, 1), pp_train.reshape(-1, 1)]
    ...
    _ = evaluate_final_model(model, test_data, y_test, hr_test, time_test, "Test set", save_dir)

    # 10. 保存模型（同时导出为ONNX格式：ONNX内自带标准化）
    model.save(f"{save_dir}/resnet_se_lstm_model.keras")
